...
<div class="course-template-activity-form"
     x-data="{ activityType: @js($selectedActivityType), completionRule: @js($selectedCompletionRule), unlockRule: @js($selectedUnlockRule), completionRules: @js($completionRules) }">
    ...
    <div class="lf-form-group">
        ...
        <select id="activity_type" name="activity_type" class="lf-form-control" x-model="activityType" @change="completionRule = (completionRules[activityType] || [])[0] || ''" required>
            <option value="" disabled>{{ __('lf.LF_course_template_activity_common_select_type') }}</option>
    ...
    <div class="lf-form-group" x-show="activityType === 'video'" x-cloak><x-form-label for="activity_video_file" value="Video file" /><input id="activity_video_file" type="file" name="activity_video_file" class="lf-form-control" accept="video/*"></div>
    ...
    <div class="lf-form-group" x-show="activityType === 'audio'" x-cloak><x-form-label for="activity_audio_file" value="Audio file" /><input id="activity_audio_file" type="file" name="activity_audio_file" class="lf-form-control" accept="audio/*"></div>
    <div class="lf-form-group" x-show="activityType === 'document'" x-cloak><x-form-label for="activity_document_file" value="Document file" /><input id="activity_document_file" type="file" name="activity_document_file" class="lf-form-control" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,application/pdf"></div>
    ...
    <div class="lf-form-group"><x-form-label for="completion_rule" :value="__('lf.LF_course_template_activity_common_completion_rule')" :required="true" /><select id="completion_rule" name="completion_rule" class="lf-form-control" x-model="completionRule" required><template x-for="rule in (completionRules[activityType] || [])"><option :value="rule" x-text="rule"></option></template></select></div>
