...
<div class="course-template-activity-form"
    ...
    <div class="lf-form-group" x-show="activityType === 'video'" x-cloak>
        ...
        <input id="activity_video_file" type="file" name="activity_video_file" class="lf-form-control authoring-media-upload" accept="video/*">
    ...
    <div class="lf-form-group" x-show="activityType === 'audio'" x-cloak>
        ...
        <input id="activity_audio_file" type="file" name="activity_audio_file" class="lf-form-control authoring-media-upload" accept="audio/*">
        <x-upload-hint :formats="['MP3', 'WAV', 'M4A', 'AAC', 'OGG']" />
    </div>
    <div class="lf-form-group" x-show="activityType === 'document'" x-cloak>
        ...
        <input id="activity_document_file" type="file" name="activity_document_file" class="lf-form-control authoring-media-upload" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,application/pdf">
    ...
    <div class="lf-form-group" x-show="['watch_percent', 'pass'].includes(completionRule)" x-cloak><x-form-label for="completion_threshold" :value="__('lf.LF_course_template_activity_common_completion_threshold')" /><input id="completion_threshold" type="number" min="0" max="100" name="completion_threshold" class="lf-form-control" value="{{ old('completion_threshold', $formActivity?->completion_threshold) }}" placeholder="{{ __('lf.LF_course_template_activity_placeholder_completion_threshold_percent') }}"></div>
    ...
    <div class="lf-form-group"><x-form-label for="unlock_rule" :value="__('lf.LF_course_template_activity_common_unlock_rule')" :required="true" /><select id="unlock_rule" name="unlock_rule" class="lf-form-control" x-model="unlockRule" :class="{ 'lf-select-placeholder': unlockRule === '' }" required><option value="" disabled>{{ __('lf.LF_course_template_activity_placeholder_unlock_rule') }}</option>@foreach (['none','previous_activity_completed','date_based'] as $rule)<option value="{{ $rule }}">{{ __('lf.LF_course_template_activity_common_unlock_'.$rule) }}</option>@endforeach</select></div>
