...
* Một activity progress luôn thuộc `template_version_id` của Enrollment.
...
* Một student chỉ có một progress record cho một activity trong cùng một product.
...
```sql
INDEX idx_course_activity_progress_template_version
