...
class CourseTemplatePublishGraphValidator
{
        ...
        foreach ($activities as $activity) {
            ...
            foreach ($mediaByActivity->get($activity->id, collect()) as $media) {
                ...
                if ((int) $media->media_customer_id !== $customerId || $media->media_status !== 'ready' || $media->usage_status !== 'active' || ($media->usage_type !== 'attachment' && ($media->usage_type !== $expectedType || ! $this->mediaTypeMatches($expectedType, $media)))) {
                    $this->failActivityMedia($activity, $lessons);
                }
            }
    ...
    private function mediaTypeMatches(string $expectedType, object $media): bool
    {
        ...
        return $media->file_type === $expectedType
            && in_array($media->mime_type, $mimes[$expectedType] ?? [], true);
    }

    private function fail(string $code): never
    {
        throw ValidationException::withMessages(['publish' => __('lf.LF_course_template_publish_integrity_'.$code)]);
    }

    private function failActivityMedia(object $activity, Collection $lessons): never
    {
        $lesson = $lessons->get($activity->template_lesson_id);

        throw ValidationException::withMessages([
            'publish' => __('lf.LF_course_template_publish_integrity_activity_media', [
