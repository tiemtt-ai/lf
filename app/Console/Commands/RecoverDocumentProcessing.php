...
class RecoverDocumentProcessing extends Command
{
    protected $signature = 'media:recover-document-processing {--customer= : Limit recovery to one tenant}';

    protected $description = 'Recover expired Document jobs and redeliver durable pending work; never deletes storage.';
    ...
    public function handle(): int
    {
        ...
        // Longer than worker timeout and the existing provider retry delay.
        $pendingCutoff = now()->subSeconds(3900);
        $recovered = 0;
        $errors = 0;
        DB::table('media_processing_jobs')->whereIn('job_type', ['ocr', 'structured_extraction'])
            ...
            ->where(fn ($q) => $q->where(fn ($p) => $p->where('status', 'pending')->where('updated_at', '<=', $pendingCutoff))
                ->orWhere(fn ($p) => $p->where('status', 'processing')
                    ->whereRaw('COALESCE(started_at, updated_at) <= ?', [$cutoff])))
                    ...
                    try {
                        DB::transaction(function () use ($candidate, $cutoff, $pendingCutoff, &$recovered): void {
                            ...
                            if ($job->status === 'processing' && ($job->started_at ?? $job->updated_at) <= $cutoff->toDateTimeString()) {
                                DB::table('media_processing_jobs')->where('customer_id', $job->customer_id)->where('id', $job->id)->update([
                                    'status' => 'failed', 'error_code' => 'provider_timeout', 'error_message' => 'provider_timeout',
                                    'completed_at' => now(), 'updated_at' => now(),
                                ]);
                                // No retry row or storage purge here. The terminal
                                // writer guard rejects late output; existing retry
                                // and manual storage cleanup policies still apply.
                                $recovered++;
                            } elseif ($job->status === 'pending' && $job->updated_at <= $pendingCutoff->toDateTimeString()) {
                                DB::table('media_processing_jobs')->where('customer_id', $job->customer_id)->where('id', $job->id)
                        ...
                        $errors++;
                        Log::error('Document recovery failed.', ['customer_id' => (int) $candidate->customer_id,
                            'processing_job_id' => (int) $candidate->id, 'exception' => $exception::class]);
                    }
                }
            });
        $this->info("Recovered {$recovered} Document job(s); {$errors} error(s).");
